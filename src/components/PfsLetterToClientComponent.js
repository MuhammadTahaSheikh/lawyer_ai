...
const PfsLetterToClientComponent = ({ caseId, nameByUid }) => {
  ...
  const startPfsPolling = (intervalMs = 2000) => {
    ...
    pfsPollRef.current = setInterval(async () => {
      try {
        const resp = await axios.get('/automations/pfs_letter_to_client', { params: { caseId } });
        ...
        console.warn('PFS Letter to Client polling failedd', e);
      }
    }, intervalMs);
  };
  ...
  const fetchPfsData = async () => {
    ...
    setPfsError(null);
    try {
      const response = await axios.get('/automations/pfs_letter_to_client', { params: { caseId } });
      if (response.data.success) {
        setPfsData(response.data.data);
      } else {
        setPfsError(response.data.message || 'Failed to fetch PFS Letter to Client data');
      }
    } catch (err) {
      console.error('Fetch PFS Letter to Client data failed', err);
      setPfsError('Failed to fetch PFS Letter to Client data');
    } finally {
  ...
  // Only show n8n-created PFS offer documents (pfs_offer_to_client_<id>.docx), ordered new to old
  const documentsForAttach = React.useMemo(() => {
    const re = /^pfs_offer_to_client_(\d+)\.docx$/i;
    return documents
      .filter(d => re.test(d.fileName || ''))
      .map(d => ({ doc: d, id: parseInt((d.fileName || '').match(re)?.[1] || '0', 10) }))
      .sort((a, b) => b.id - a.id)
      .map(({ doc }) => doc);
  }, [documents]);
  ...
  const handleTriggerPfs = async () => {
    ...
    try {
      ...
      const triggerResp = await axios.post('/automations/pfs_letter_to_client/trigger', payload, {
        withCredentials: true,
      ...
      console.error('Trigger PFS Letter to Client failed', err);
      setPfsError('Failed to trigger PFS Letter to Client automation');
    } finally {
  ...
  const handleSavePfs = async () => {
    try {
      const resp = await axios.post('/automations/pfs_letter_to_client', { 
        caseId, 
        ...(pfsData || {}), 
        uid: currentUserUid 
      });
      if (resp.data.success) {
        alert('PFS Letter to Client saved');
      } else {
        alert(`Error: ${resp.data.message}`);
      }
    } catch (err) {
      console.error('Save PFS Letter to Client failed', err);
      alert('Save PFS Letter to Client failed');
    }
  };
  ...
  const handleTriggerPfsUiPath = async () => {
    ...
    try {
      ...
      const resp = await axios.post('/automations/pfs_letter_to_client/queue', payload, {
        withCredentials: true,
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (resp.data?.success) {
        ...
        alert(`Error starting PFS Letter to Client UiPath automation: ${resp.data?.message || 'unknown error'}`);
      }
    } catch (err) {
      console.error('Trigger PFS Letter to Client UiPath failed', err);
      setPfsError('Failed to trigger PFS Letter to Client UiPath');
    } finally {
      setTriggeringPfs(false);
    }
  };
  ...
  const handleRerunPfs = async () => {
    if (!window.confirm('This will clear existing PFS Letter to Client data and re-trigger. Continue?')) return;
    setPfsError(null);
    setTriggeringPfs(true);
    try {
      await axios.post('/automations/pfs_letter_to_client/rerun', { caseId, uid: currentUserUid });
      for (let i = 0; i < 40; i++) {
      ...
      console.error('Re-run PFS Letter to Client failed', err);
      setPfsError('Failed to re-run PFS Letter to Client automation');
    } finally {
      setTriggeringPfs(false);
    }
  };
  ...
  const handleResetPfsStatus = async () => {
    try {
      await axios.put('/automations/pfs_letter_to_client', { caseId, status: 'pending' });
      setPfsData(prev => ({ ...(prev || {}), status: 'pending' }));
    } catch (err) {
      console.error('Reset PFS Letter to Client status failed', err);
      alert('Failed to reset PFS Letter to Client status to pending');
    }
  };

  // Reset and clear data to start fresh (back to trigger part)
  const handleResetPfs = async () => {
    if (!window.confirm('Are you sure you want to reset the PFS Letter to Client automation?')) return;
    setPfsError(null);
    setTriggeringPfs(true);
    try {
      await axios.delete('/automations/pfs_letter_to_client', { params: { caseId } });
      await fetchPfsData();
    } catch (err) {
      console.error('Reset PFS Letter to Client failed', err);
      setPfsError('Failed to reset PFS Letter to Client automation');
    } finally {
      setTriggeringPfs(false);
    }
  };
  ...
  if (loadingPfsData || (triggeringPfs && !pfsData)) {
    return (
      <Box sx={{ mt: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
        ...
        <Typography>Loading PFS Letter to Client data…</Typography>
      </Box>
    );
  }
    ...
    return (
      <Box sx={{ mt: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center', mt: 1 }}>
          <Button variant="outlined" color="neutral" onClick={handleResetPfs}>
            Reset
          </Button>
        </Box>
      </Box>
    ...
    <Box sx={{ mt: 2, maxWidth: 720 }}>
      ...
      {showFailed && (
        <Box sx={{ mt: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          ...
          <Typography level="body2" sx={{ opacity: 0.7, textAlign: 'center', maxWidth: 520 }}>
            The PFS Letter to Client automation encountered an error. You can try again or reset the status.
          </Typography>
      ...
      {showCompleted && (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          ...
          <Typography>PFS Letter to Client automation completed successfully</Typography>
      ...
      {showForm && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Typography level="h5" sx={{ mb: 1 }}>PFS Letter to Client</Typography>
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
            ...
            >
              Re-run PFS Letter to Client Automation
            </Button>
          </Box>
        </Box>
      )}
...
export default PfsLetterToClientComponent;
