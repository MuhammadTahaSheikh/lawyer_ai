...
import { Box, Typography, Input, Button, FormControl, FormLabel, Select, Option, Sheet, Checkbox, Divider, Table, Textarea } from '@mui/joy';
...
const DemandToOcComponent = ({ caseId, nameByUid }) => {
  ...
  const [phase2DocSearchQuery, setPhase2DocSearchQuery] = useState('');
  ...
  const automationEndpoint = '/automations/presuit_demand';
  const automationLabel = 'Presuit Demand';
    ...
    try {
      ...
      if (resp.data.success) {
        ...
        if (triggerPhase2) {
          ...
          console.log('[DemandToOc] Phase 2 webhook response:', phase2Resp?.data);
        }
      ...
      console.error('[DemandToOc] Save request failed details:', err?.response?.data || err?.message || err);
  ...
  const phase2DocSearchLower = (phase2DocSearchQuery || '').trim().toLowerCase();
  const filteredPhase2Documents = phase2DocSearchLower
    ? documents.filter((d) => {
        const fileName = (d.fileName || '').toLowerCase();
        const folder = (d.folder || '').toLowerCase();
        return fileName.includes(phase2DocSearchLower) || folder.includes(phase2DocSearchLower);
      })
    : documents;

  const toggleAllFilteredPhase2DocKeys = () => {
    const allSelected = filteredPhase2Documents.length > 0 &&
      filteredPhase2Documents.every((d) => selectedDocs.includes(d.key));
    if (allSelected) {
      setSelectedDocs((prev) => prev.filter((k) => !filteredPhase2Documents.some((d) => d.key === k)));
    } else {
      setSelectedDocs((prev) => [...new Set([...prev, ...filteredPhase2Documents.map((d) => d.key)])]);
    }
  };

  // Phase 3 documents: show all case documents, ordered new to old by trailing id when present
  ...
  const handleSendAllToUiPath = async () => {
    ...
    const payload = {
      ...
      type_of_loss: responseToMdtData?.type_of_loss || '',
      pa_estimate: responseToMdtData?.pa_estimate ?? '',
      deductible: responseToMdtData?.deductible ?? '',
      prior_payment: responseToMdtData?.prior_payment ?? '',
      aob_dtp_invoices: responseToMdtData?.aob_dtp_invoices ?? '',
      date_of_loss: responseToMdtData?.date_of_loss || '',
      claim_number: responseToMdtData?.claim_number || '',
      policy_number: responseToMdtData?.policy_number || '',
      premises: responseToMdtData?.premises || '',
      ai_response: responseToMdtData?.ai_response || '',
      ...
      oc_email: responseToMdtData?.oc_email || '',
    };
      ...
      {showFiled && (
        <>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1, flexWrap: 'wrap' }}>
            <Typography level="title-sm">All case documents</Typography>
            <Button
              variant="outlined"
              size="sm"
          ...
          {documents.length > 0 && (
            <Input
              ...
              value={phase2DocSearchQuery}
              onChange={(e) => setPhase2DocSearchQuery(e.target.value)}
              sx={{ mb: 1, flex: '1 1 200px', minWidth: 180 }}
              size="sm"
            />
          )}
          <Sheet variant="outlined" sx={{ p: 1, borderRadius: 8, mb: 2, maxHeight: 320, overflow: 'auto' }}>
            {documents.length === 0 ? (
              ...
              <>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  <Checkbox
                    checked={filteredPhase2Documents.length > 0 && filteredPhase2Documents.every((d) => selectedDocs.includes(d.key))}
                    indeterminate={
                      filteredPhase2Documents.some((d) => selectedDocs.includes(d.key)) &&
                      !filteredPhase2Documents.every((d) => selectedDocs.includes(d.key))
                    }
                    onChange={toggleAllFilteredPhase2DocKeys}
                    label="Select all"
                  />
                  <Typography level="body3" sx={{ ml: 'auto' }}>
                    {selectedDocs.length} selected
                    {phase2DocSearchQuery.trim() && (
                      <Typography component="span" level="body3" sx={{ opacity: 0.7 }}>
                        {' '}({filteredPhase2Documents.length} match)
                      </Typography>
                    )}
                  </Typography>
                </Box>
                {filteredPhase2Documents.length === 0 ? (
                  <Typography level="body2" sx={{ opacity: 0.8 }}>
                    No documents match &quot;{phase2DocSearchQuery.trim()}&quot;.
                  </Typography>
                ) : (
                filteredPhase2Documents.map((d) => (
                  <Box key={`phase2-${d.key}`} sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 0.5 }}>
                    <Checkbox
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1, mb: 2 }}>
            ...
            <Button variant="outlined" onClick={handleSendAllToUiPath} disabled={sendingAll || selectedDocs.length === 0}>
          ...
          <Box sx={{ display: 'grid', gap: 1.5, mb: 2 }}>
            <FormControl>
              ...
              <Select
                value={responseToMdtData?.type_of_loss || ''}
                onChange={(event, value) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), type_of_loss: value || '' }))
                }
              >
            ...
            <FormControl>
              ...
              <Input
                type="text"
                value={responseToMdtData?.pa_estimate ?? ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), pa_estimate: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              ...
              <Input
                type="text"
                value={responseToMdtData?.deductible ?? ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), deductible: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              <FormLabel>Prior Payment</FormLabel>
              <Input
                type="text"
                value={responseToMdtData?.prior_payment ?? ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), prior_payment: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              <FormLabel>AOB-DTP Invoices</FormLabel>
              <Input
                type="text"
                value={responseToMdtData?.aob_dtp_invoices ?? ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), aob_dtp_invoices: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              ...
              <Input
                value={responseToMdtData?.date_of_loss || ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), date_of_loss: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              ...
              <Input
                value={responseToMdtData?.claim_number || ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), claim_number: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              ...
              <Input
                value={responseToMdtData?.policy_number || ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), policy_number: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              ...
              <Input
                value={responseToMdtData?.premises || ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), premises: e.target.value }))
                }
              />
            </FormControl>
            <Box>
      ...
      {showFiledTwo && (
        <>
          ...
          <Sheet variant="outlined" sx={{ p: 1, borderRadius: 8, mb: 2, maxHeight: 260, overflow: 'auto' }}>
            {documentsForAttach.length === 0 ? (
              <Typography level="body2" sx={{ opacity: 0.7 }}>{`No n8n-generated ${automationLabel} document found for this case. Trigger the automation to create one.`}</Typography>
            ) : (
              <>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  <Checkbox
          ...
          <Divider sx={{ my: 3 }} />
          <Typography level="title-sm" sx={{ mb: 1 }}>Email fields</Typography>
          <Box sx={{ display: 'grid', gap: 1.5, mb: 2 }}>
            <FormControl>
              <FormLabel>Defendant Email</FormLabel>
              <Input
                type="email"
            ...
            <FormControl>
              <FormLabel>OC Email</FormLabel>
              <Input
                type="email"
                value={responseToMdtData?.oc_email || ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), oc_email: e.target.value }))
                }
              />
            </FormControl>
            <FormControl>
              ...
              <Textarea
                minRows={4}
                value={responseToMdtData?.ai_response || ''}
                onChange={(e) =>
                  setResponseToMdtData((prev) => ({ ...(prev || {}), ai_response: e.target.value }))
                }
              />
            </FormControl>
            <Box>
...
export default DemandToOcComponent;
