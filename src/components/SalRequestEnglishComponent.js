...
const SalRequestEnglishComponent = ({ caseId, nameByUid }) => {
  ...
  const startSalPolling = (intervalMs = 2000) => {
    ...
    salPollRef.current = setInterval(async () => {
      try {
        const resp = await axios.get('/automations/sal_request_english', { params: { caseId } });
        ...
        console.warn('SAL Request English polling failed', e);
      }
    }, intervalMs);
  };
  ...
  const fetchSalData = async () => {
    ...
    setSalError(null);
    try {
      const response = await axios.get('/automations/sal_request_english', { params: { caseId } });
      if (response.data.success) {
        setSalData(response.data.data);
      } else {
        setSalError(response.data.message || 'Failed to fetch SAL Request English data');
      }
    } catch (err) {
      console.error('Fetch SAL Request English data failed', err);
      setSalError('Failed to fetch SAL Request English data');
    } finally {
  ...
  const handleTriggerSal = async () => {
    ...
    try {
      ...
      const triggerResp = await axios.post('/automations/sal_request_english/trigger', payload, {
      ...
      console.error('Trigger SAL Request English failed', err);
      setSalError('Failed to trigger SAL Request English automation');
    } finally {
  ...
  const handleSaveSal = async () => {
    try {
      const resp = await axios.post('/automations/sal_request_english', { 
        caseId, 
        ...(salData || {}), 
        uid: currentUserUid 
      });
      if (resp.data.success) {
        alert('SAL Request English saved');
      } else {
        alert(`Error: ${resp.data.message}`);
      }
    } catch (err) {
      console.error('Save SAL Request English failed', err);
      alert('Save SAL Request English failed');
    }
  };
  ...
  const handleTriggerSalUiPath = async () => {
    ...
    try {
      ...
      const resp = await axios.post('/automations/sal_request_english/queue', payload, {
        withCredentials: true,
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (resp.data?.success) {
        ...
        alert(`Error starting SAL Request English UiPath automation: ${resp.data?.message || 'unknown error'}`);
      }
    } catch (err) {
      console.error('Trigger SAL Request English UiPath failed', err);
      setSalError('Failed to trigger SAL Request English UiPath');
    } finally {
      setTriggeringSal(false);
    }
  };
  ...
  const handleRerunSal = async () => {
    if (!window.confirm('This will clear existing SAL Request English data and re-trigger. Continue?')) return;
    setSalError(null);
    setTriggeringSal(true);
    try {
      await axios.post('/automations/sal_request_english/rerun', { caseId, uid: currentUserUid });
      for (let i = 0; i < 40; i++) {
      ...
      console.error('Re-run SAL Request English failed', err);
      setSalError('Failed to re-run SAL Request English automation');
    } finally {
      setTriggeringSal(false);
    }
  };
  ...
  const handleResetSalStatus = async () => {
    try {
      await axios.put('/automations/sal_request_english', { caseId, status: 'pending' });
      setSalData(prev => ({ ...(prev || {}), status: 'pending' }));
    } catch (err) {
      console.error('Reset SAL Request English status failed', err);
      alert('Failed to reset SAL Request English status to pending');
    }
  };

  // Reset and clear data to start fresh (back to trigger part)
  const handleResetSal = async () => {
    if (!window.confirm('Are you sure you want to reset the SAL Request English automation?')) return;
    setSalError(null);
    setTriggeringSal(true);
    try {
      await axios.delete('/automations/sal_request_english', { params: { caseId } });
      await fetchSalData();
    } catch (err) {
      console.error('Reset SAL Request English failed', err);
      setSalError('Failed to reset SAL Request English automation');
    } finally {
      setTriggeringSal(false);
    }
  };
  ...
  if (loadingSalData || (triggeringSal && !salData)) {
    return (
      <Box sx={{ mt: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
        ...
        <Typography>Loading SAL Request English data…</Typography>
      </Box>
    );
  }
    ...
    <Box sx={{ mt: 2, maxWidth: 720 }}>
      ...
      {showFailed && (
        <Box sx={{ mt: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          ...
          <Typography level="body2" sx={{ opacity: 0.7, textAlign: 'center', maxWidth: 520 }}>
            The SAL Request English automation encountered an error. You can try again or reset the status.
          </Typography>
      ...
      {showCompleted && (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          ...
          <Typography>SAL Request English automation completed successfully</Typography>
      ...
      {showForm && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Typography level="h5" sx={{ mb: 1 }}>SAL Request English</Typography>
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
            ...
            >
              Re-run SAL Request English Automation
            </Button>
          </Box>
        </Box>
      )}
...
export default SalRequestEnglishComponent;
